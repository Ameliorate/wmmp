...
mpd_Connection *mpd_newConnection(const char *host, int port, float timeout)
{
        ...
        /* connect stuff */
        {
                ...
                if (getsockopt
                    (connection->sock, SOL_SOCKET, SO_RCVTIMEO, &rcvoldto,
                     &oldlen) < 0
                    || getsockopt(connection->sock, SOL_SOCKET, SO_SNDTIMEO,
                                  &sndoldto, &oldlen) < 0) {
                        strcpy(connection->errorStr,
                ...
                if (setsockopt(connection->sock, SOL_SOCKET, SO_RCVTIMEO, &tv,
                    ...
                    setsockopt(connection->sock, SOL_SOCKET,
                               SO_SNDTIMEO, &tv, sizeof(struct timeval)) < 0) {
...
#ifdef SO_RCVTIMEO
                if (setsockopt
                    (connection->sock, SOL_SOCKET, SO_SNDTIMEO, &rcvoldto,
                     sizeof(struct timeval)) < 0
                    || setsockopt(connection->sock, SOL_SOCKET, SO_SNDTIMEO,
                                  &sndoldto, sizeof(struct timeval)) < 0) {
                        strcpy(connection->errorStr,
        ...
        if (connection->returnElement) {
                if (strcmp(connection->returnElement->name, "file") == 0) {
                        ...
                        entity->info.directory->path =
                            strdup(connection->returnElement->value);
                } else if (strcmp(connection->returnElement->name, "playlist")
                           == 0) {
                        entity = mpd_newInfoEntity();
                         ...
                         const char *arg1)
{
        ...
        if (arg1) {
                ...
                string =
                    malloc(strlen("list") + strlen(sanitArg1) + strlen(st) + 6);
