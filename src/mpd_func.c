...
void MpdPrev()
{
        if (status->elapsedTime > 5) {
                mpd_sendSeekCommand(connection, status->song, 0);
        } else {
...
void MpdFastr()
{
        if (!status || status->state == MPD_STATUS_STATE_STOP)
                return;
...
char *MpdGetTitle()
{
        ...
        if (ret && lastVersion == status->playlist && lastSong == status->song) {
                return ret;
        }
        ...
        if ((entity = mpd_getNextInfoEntity(connection))) {
                if (entity->info.song->artist && entity->info.song->title) {
                        ...
                        ret = strdup(entity->info.song->file);
