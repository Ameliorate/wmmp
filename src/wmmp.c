...
static char *transliterate(char *s);
static char *strip_extra_spaces(char *s);
...
int main(int argc, char **argv)
{
        ...
        if (!dockapp_xpm2pixmap
            (backlight_on_xpm, &backdrop_on, &mask, colors, ncolor)) {
                fprintf(stderr,
                        "Error initializing back-lit background image.\n");
                exit(1);
        }
        if (!dockapp_xpm2pixmap
            (backlight_off_xpm, &backdrop_off, NULL, NULL, 0)) {
        ...
        while (1) {
                if (dockapp_nextevent_or_timeout(&event, update_interval * 200)) {
                        ...
                        case ButtonRelease:
                                left_down = 0;
                                unhighlight_button(event.xbutton.x,
                                                   event.xbutton.y, 0);
                                break;
                        case MotionNotify:
        ...
        if (cur_mpd_infos.running) {
                ...
                case 0:
                        if (cur_mpd_infos.repeat != 1)
                                dockapp_copyarea(parts, pixmap, 135, 81, 10, 6,
                                                 44, 5);
                        break;
                case 1:
                        if (cur_mpd_infos.shuffling != 1)
                                dockapp_copyarea(parts, pixmap, 148, 81, 10, 6,
                                                 44, 12);
                        break;
                case 2:
                ...
                case 5:
                        ...
                        break;
                case 6:
                        dockapp_copyarea(parts, pixmap, 105, q + 13, 9, 9, 4,
                                         45);
                        break;
                case 7:
                        if (!cur_mpd_infos.playing) {
                                triangle_loc = 3;
                                dockapp_copyarea(parts, pixmap, 114, q + 13, 21,
                                                 9, 13, 45);
                        }
                        break;
                case 8:
                        if (!cur_mpd_infos.paused)
                                dockapp_copyarea(parts, pixmap, 135, q + 13, 10,
                                                 9, 34, 45);
                        break;
                case 9:
                        dockapp_copyarea(parts, pixmap, 145, q + 13, 10, 9, 44,
                                         45);
                        break;
                }
        }
...
static void update_triangle()
{
        ...
        dockapp_copyarea(parts, pixmap, (x + (triangle_loc * 8)), y, 6, 6, 34,
                         20);
        triangle_loc++;
        dockapp_copy2window(pixmap);
...
static void update_track(int trk)
{
        ...
        for (; i < 3; i++) {
                dockapp_copyarea(parts, pixmap, (*p - '0') * 6, y, 6, 7,
                                 (i * 6) + 38, 20);
                p++;
...
static void update_time(int tme)
{
        ...
        if (tme < 6000) {
                ...
                if (tme < 360000) {
                        sprintf(timestr, "%02d%02d", tme / 3600,
                                tme % 3600 / 60);
                } else {
        ...
        for (; i < 4; i++) {
                dockapp_copyarea(parts, pixmap, (*p - '0') * 5 + x, y, 5, 9,
                                 i < 2 ? (i * 6) + 5 : (i * 6) + 7, 18);
                p++;
        }
}
...
static void update_title(char *name)
{
        ...
        char *title_new = transliterate(title);
        char *title_new2 = strip_extra_spaces(title_new);
        free(title_new);
        title_new = title_new2;
        len = strlen(title_new);
        ...
        for (i = 0; i < max; i++) {
                {
                        ...
                        if (tmp < len)
                                c = toupper(title_new[tmp]);
                        else
                                c = toupper(title_new[(tmp - len) % 6]);
                }

                if (isalpha(c))
                        dockapp_copyarea(parts, pixmap, (c - 'A') * 6, 72 + y,
                                         6, 8, (i * 6) + 7, 7);
                else if (isdigit(c))
                        dockapp_copyarea(parts, pixmap, (c - '0') * 6, 81 + y,
                                         6, 8, (i * 6) + 7, 7);
                else if (isspace(c))
                        dockapp_copyarea(parts, pixmap, 60, 81 + y, 6, 8,
                                         (i * 6) + 7, 7);
                else {
                        switch (c) {
                        case '-':
                                dockapp_copyarea(parts, pixmap, 66, 81 + y, 6,
                                                 8, (i * 6) + 7, 7);
                                break;
                        case '.':
                                dockapp_copyarea(parts, pixmap, 72, 81 + y, 6,
                                                 8, (i * 6) + 7, 7);
                                break;
                        case 0x27:
                                dockapp_copyarea(parts, pixmap, 79, 81 + y, 6,
                                                 8, (i * 6) + 7, 7);
                                break;
                        case '(':
                                dockapp_copyarea(parts, pixmap, 84, 81 + y, 6,
                                                 8, (i * 6) + 7, 7);
                                break;
                        case ')':
                                dockapp_copyarea(parts, pixmap, 90, 81 + y, 6,
                                                 8, (i * 6) + 7, 7);
                                break;
                        case '*':
                                dockapp_copyarea(parts, pixmap, 96, 81 + y, 6,
                                                 8, (i * 6) + 7, 7);
                                break;
                        case '/':
                                dockapp_copyarea(parts, pixmap, 103, 81 + y, 6,
                                                 8, (i * 6) + 7, 7);
                                break;
                        default:
                                dockapp_copyarea(parts, pixmap, 60, 81 + y, 6,
                                                 8, (i * 6) + 7, 7);
                                break;
                        }
                }
        ...
        for (; i < 6; i++) {
                dockapp_copyarea(parts, pixmap, 60, 81 + y, 6, 8, (i * 6) + 7,
                                 7);
        }
        if (len > 6)
...
static void update()
{
        ...
        if (cur_mpd_infos.running) {
                ...
                if (backlight == LIGHTON)
                        dockapp_copyarea(backdrop_on, pixmap, 0, 0, 58, 58, 0,
                                         0);
                else
                        dockapp_copyarea(backdrop_off, pixmap, 0, 0, 58, 58, 0,
                                         0);
                triangle_loc = 3;
                mpd->playing = 0;
                update_slider(25);
...
{
        int len = strlen(s);
        char *kakasi_out_path = "/tmp/wmmp_kakasi_out";
        char *kakasi_in_path = "/tmp/wmmp_kakasi_in";
        char command[200 + sizeof(kakasi_in_path) + sizeof(kakasi_out_path)] =
            { 0 };
        // make sure to increase size of buffer if the command changed.
        sprintf(command,
                "cat %s | iconv -f utf8 -t eucjp | kakasi -i euc -w | kakasi -i euc -Ha -Ka -Ja -Ea -ka > %s",
                kakasi_in_path, kakasi_out_path);

        FILE *kakasi_in = fopen(kakasi_in_path, "w");
        if (!kakasi_in_path) {
                fprintf(stderr,
                        "Error opening input file transliterating string: %m\n");
                exit(1);
        }
        fputs(s, kakasi_in);
        fputc('\n', kakasi_in);

        if (fclose(kakasi_in) == EOF) {
                fprintf(stderr,
                        "Error closing input file transliterating string: %m\n");
                exit(1);
        }

        int kresult = system(command);
        //if (kresult = -1) { // Always returns -1 for some reason.
        if (0) {
                fprintf(stderr, "Error running kakasi: %m\n");
                exit(1);
        }
        //if (!kresult) {
        if (0) {
                fprintf(stderr,
                        "Error while transliterating string \"%s\", kakasi returned %d\n",
                        s, kresult);
                exit(kresult);
        }

        FILE *kakasi_out;
        if (!(kakasi_out = fopen(kakasi_out_path, "r"))) {
                fprintf(stderr, "Error opening output file from kakasi: %m\n");
                exit(1);
        }

        char *s_new = malloc(len * 16); // Kanji (plural?) can be really long when made to romanji
        if (!s_new) {
                puts("Out of memory.");
                exit(1);
        }

        if (!fgets(s_new, len * 16, kakasi_out)) {
                fprintf(stderr,
                        "Unexpected EOF reading output file from kakasi\n");
                exit(1);
        }
        if (fclose(kakasi_out) == EOF) {
                fprintf(stderr,
                        "Error closing output file transliterating string: %m\n");
                exit(1);
        }

        if (remove(kakasi_out_path) == -1) {
                fprintf(stderr, "Error deleting kakasi output file: %m");
                exit(1);
        }
        if (remove(kakasi_in_path) == -1) {
                fprintf(stderr, "Error deleting kakasi input file: %m");
                exit(1);
        }
...
static char *strip_extra_spaces(char *s)
{
        ...
        int last_was_apost = 0;
        for (int i = 0; s[i] != '\0'; i++) {
                int is_space = s[i] == ' ';
                if ((last_was_space || last_was_paren || last_was_slash
                     || last_was_apost) && is_space) {
                        goto end;
                }
